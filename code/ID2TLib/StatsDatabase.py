...
class StatsDatabase:
    ...
    def _process_named_query(self, query_param_list):
        ...
        named_queries = {
            ...
            "most_used.ttlvalue": "SELECT ttlValue FROM (SELECT ttlValue, SUM(ttlCount) as occ FROM ip_ttl GROUP BY ttlValue ORDER BY SUM(ttlCount) DESC) WHERE occ=(SELECT SUM(ttlCount) as occ FROM ip_ttl GROUP BY ttlValue ORDER BY occ DESC LIMIT 1) ORDER BY ttlValue ASC",
