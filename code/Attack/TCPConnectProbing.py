...
class TCPConnectProbing(BaseAttack.BaseAttack):
    ...
    def __init__(self):
        """
        Creates a new instance of the TCP Connect Probing.
        This attack injects TCP Connect-requests and respective responses into the output pcap file.
        """
        # Initialize attack
        super(TCPConnectProbing, self).__init__("TCPConnectProbing", "Injects a nmap 'Connect scan'",
                                             "Scanning/Probing")
    ...
    def generate_attack_packets(self):
        ...
        for ip in ip_destination:
            ...
            for dport in dest_ports:
                ...
                # SYN
                request_tcp = inet.TCP(sport=sport, dport=dport, seq=src_starting_seq, window=source_win_value, flags='S', 
                                options=[('MSS', source_mss_value), ('SAckOK', ''),
                                         ('Timestamp', (src_tsval, 0)), ('NOP', ''), ('WScale', CONNECT_WSCALE)])
                ...
                if dport not in ports_open:  # destination port is CLOSED
                    ...
                    reply_tcp = inet.TCP(sport=dport, dport=sport, seq=dst_starting_seq, ack=src_starting_seq+1, flags='SA', window=destination_win_value,
                                options=[('MSS', destination_mss_value), ('SAckOK', ''), ('Timestamp', (dst_tsval, src_tsval)), ('NOP', ''), ('WScale', CONNECT_WSCALE)])
                    ...
                    reset.time = timestamp_reset
                    self.add_packet(reset, ip_source, ip)
