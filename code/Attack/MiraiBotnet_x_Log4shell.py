...
class MiraiBotnet_x_Log4shell(BaseAttack.BaseAttack):
    ...
    TIME_AFTER_FIRST_STAGE = 'time.after.first.stage'
    TIME_AFTER_SECOND_STAGE = 'time.after.second.stage'

    ATTACKER_BOTNET_IP = '172.16.0.1'
    LOADER_SERVER_IP = '113.15.107.192'
    CNC_SERVER_IP = '49.193.199.35'
    VICTIM_IP = '172.16.0.2'
    HTTP_FLOOD_TARGET_IP = '42.123.199.35'
    INIT_PORT = 44938
    HTTP_PORT = 80
    FTP_PORT = 23
    LOADER_PORT = 23
    template_attack_pcap_path = Util.RESOURCE_DIR + "Mirai-x-Log4j.pcap"
    ...
    def __init__(self):
        ...
        """
        # Initialize attack
        super(MiraiBotnet_x_Log4shell, self).__init__("MiraiBotnet_x_Log4shell", "Injects a Mirai Log4shell botnet'", "Botnet")
        ...
        self.update_params([
            ...
            Parameter(self.TIME_AFTER_FIRST_STAGE, Float()),
            Parameter(self.TIME_AFTER_SECOND_STAGE, Float()),
    ...
    def init_param(self, param: str) -> bool:
        ...
        elif param == self.TIME_AFTER_FIRST_STAGE:
            value = 0
        elif param == self.TIME_AFTER_SECOND_STAGE:
            value = 0
        if value is None:
            return False
    ...
    def generate_attack_packets(self):
        ...
        # TIME
        time_after_first_stage = self.get_param_value(self.TIME_AFTER_FIRST_STAGE)
        time_after_second_stage = self.get_param_value(self.TIME_AFTER_SECOND_STAGE)

        # Check time difference between stages: 
        time_after_first_stage, time_after_second_stage = self.calculate_and_check_time_stages(time_after_first_stage, time_after_second_stage)
        # Check ip.src == ip.dst
        ...
        for self.pkt_num, pkt in enumerate(exploit_raw_packets):
            ...
            # LOADER_SERVER OR CNC_SERVER -> VICTIM
                ...
                if ip_pkt.getfieldval("src") == self.CNC_SERVER_IP: # CNC_SERVER
                    eth_frame.setfieldval("src", mac_cnc_server)
                    ip_pkt.setfieldval("src", ip_cnc_server)
                    tcp_pkt.setfieldval("dport", ep)
                    if self.pkt_num >= 389:
                    # STAGE TIME
                        stage_time = time_after_second_stage
                    else: 
                        stage_time = time_after_first_stage
            ...
            new_pkt.time = timestamp_next_pkt + arrival_time + stage_time
    ...
    def calculate_and_check_time_stages(self, time_after_first_stage, time_after_second_stage):
        """
        Update time values for two dependent stages, ensuring the second stage time is always larger.
        Args:
            time_after_first_stage (float): Time after first stage completion.
            time_after_second_stage (float): Time after second stage completion.
        Returns:
            tuple: A tuple containing updated time values for both stages.
        """
        ...
        if time_after_first_stage > time_after_second_stage:
            print("WARNING: IGNORED both stage times because the second stage time is not larger or equal to the first stage time.")
            return 0,0
